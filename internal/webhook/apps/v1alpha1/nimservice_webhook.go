...
func (v *NIMServiceCustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	errList = append(errList, validateImageConfiguration(&nimservice.Spec.Image, field.NewPath("spec").Child("image"))...)

	if nimservice.Spec.AuthSecret == "" {
		errList = append(errList, field.Required(field.NewPath("spec").Child("authSecret"), "is required"))
	}

	errList = append(errList, validateServiceStorageConfiguration(nimservice, field.NewPath("spec").Child("storage"))...)
	...
	if nimservice.Spec.Expose.Ingress.Enabled != nil && *nimservice.Spec.Expose.Ingress.Enabled {
		if nimservice.Spec.Expose.Ingress.Spec.IngressClassName == nil && nimservice.Spec.Expose.Ingress.Spec.DefaultBackend == nil &&
			...
			errList = append(errList, field.Required(field.NewPath("spec").Child("expose").Child("ingress").Child("spec"), "must be defined if ingress.enabled is true"))
		}
	}
	...
	if nimservice.Spec.Metrics.Enabled != nil && *nimservice.Spec.Metrics.Enabled {
		if len(nimservice.Spec.Metrics.ServiceMonitor.AdditionalLabels) == 0 && len(nimservice.Spec.Metrics.ServiceMonitor.Annotations) == 0 &&
			...
			errList = append(errList, field.Required(field.NewPath("spec").Child("metrics").Child("serviceMonitor"), "must be defined if spec.metrics.enabled is true"))
		}
	}
	...
	if nimservice.Spec.Scale.Enabled != nil && *nimservice.Spec.Scale.Enabled {
		if nimservice.Spec.Scale.HPA.MinReplicas == nil && nimservice.Spec.Scale.HPA.MaxReplicas == 0 && len(nimservice.Spec.Scale.HPA.Metrics) == 0 && nimservice.Spec.Scale.HPA.Behavior == nil {
			errList = append(errList, field.Required(field.NewPath("spec").Child("scale").Child("hpa"), "must be defined if spec.scale.enabled is true"))
		}
	}
	...
	if len(nimservice.Spec.DRAResources) > 0 {
		...
		if err != nil || n < 3 {
			errList = append(errList, field.Invalid(field.NewPath("spec").Child("draResources"), v.k8sVersion, "unable to parse Kubernetes server version"))
		} else if major < 1 || (major == 1 && (minor < 33 || (minor == 33 && patch < 1))) {
			errList = append(errList, field.Forbidden(field.NewPath("spec").Child("draResources"), "Spec.DRAResources is only supported on Kubernetes v1.33.1 or newer"))
		}
	}
	...
	if nimservice.Spec.Resources != nil {
		if nimservice.Spec.Resources.Claims != nil || len(nimservice.Spec.Resources.Claims) != 0 {
			errList = append(errList, field.Forbidden(field.NewPath("spec").Child("resources").Child("claims"), "must be empty"))
		}
	}

	errList = append(errList, validateDRAResourcesConfiguration(nimservice, field.NewPath("spec").Child("draResources"))...)
...
func (v *NIMServiceCustomValidator) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	if !equality.Semantic.DeepEqual(oldNIMService.Spec.MultiNode, newNIMService.Spec.MultiNode) {
		errList = append(errList, field.Forbidden(field.NewPath("spec").Child("multiNode"), "NIMService.Spec.MultiNode is immutable once the resource is created"))
	}
	...
	if oldPVC.Create != nil && *oldPVC.Create {
		...
		if !equality.Semantic.DeepEqual(oldPVC, newPVC) {
			errList = append(errList, field.Forbidden(field.NewPath("spec").Child("storage").Child("pvc"), "NIMService.Spec.Storage.PVC is immutable once it is created with PVC.Create = true"))
		}
	}
