...
var nimcachelog = logf.Log.WithName("nimcache-resource")
...
func (v *NIMCacheCustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	nimcachelog.Info("Validation for NIMCache upon creation", "name", nimcache.GetName())

	errList := field.ErrorList{}

	errList = append(errList, validateNIMSourceConfiguration(&nimcache.Spec.Source, field.NewPath("spec").Child("source"))...)
	errList = append(errList, validateNIMCacheStorageConfiguration(&nimcache.Spec.Storage, field.NewPath("spec").Child("storage"))...)
	errList = append(errList, validateProxyConfiguration(nimcache.Spec.Proxy, field.NewPath("spec").Child("proxy"))...)
...
func (v *NIMCacheCustomValidator) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	nimcachelog.Info("Validation for NIMCache upon update", "name", nimcache.GetName())
	...
	if !equality.Semantic.DeepEqual(oldNIMCache.Spec,
		newNIMCache.Spec) {
		return nil, fmt.Errorf("NIMCache.Spec is immutable once the object is created")
	}
